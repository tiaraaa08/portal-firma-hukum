			...
			CirleProgress: function (){
				if($('.progress_area').length){
					...
					$('.first.progress_area').circleProgress({
						...
						$(this).find('strong').html(Math.round(82 * progress) + '<span>%</span>');
					});
					$('.second.progress_area').circleProgress({
						value: .78,
						thickness: 6,
						emptyFill: '#f0f0f0',
					}).on('circle-animation-progress', function(event, progress) {
						$(this).find('strong').html(Math.round(78 * progress) + '<span>%</span>');
					});
					$('.third.progress_area').circleProgress({
						value: .85,
						thickness: 6,
						emptyFill: '#f0f0f0',
					}).on('circle-animation-progress', function(event, progress) {
					...
					$('.fourth.progress_area').circleProgress({
						value: .80,
						thickness: 6,
						emptyFill: '#f0f0f0',
					}).on('circle-animation-progress', function(event, progress) {
